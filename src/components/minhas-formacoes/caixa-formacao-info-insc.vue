        ...
        <div class="grid grid-cols-10">
            <div class="col-span-4 pb-3">
                <div class="flex items-center">
                    <div class="fw-600 text-[12pt] text-green-600">
                        {{ formacao.nome }}
                        <span v-if="formacao.turmasPorArea"
                            class="ml-1 text-[6pt] fw-600 px-1 pt-[2px] rounded text-green-700 bg-slate-300">
                            Área
                        </span>
                    </div>
                </div>
                ...
                <div class="my-[3px] fw-400 text-[8pt]" v-if="formacao.dataInicio == formacao.dataTermino">
                    <span>DATA:</span>
                    <span class="ml-1 text-[10pt] fw-500 px-1 pt-[2px] rounded text-green-700 bg-slate-200">
                ...
                <div class="my-[3px] fw-400 text-[8pt]" v-if="formacao.dataInicio != formacao.dataTermino">
                    ...
                    <span class="ml-1 text-[10pt] fw-500 pl-[1px] pt-[2px] rounded text-green-700 bg-slate-200">
                        {{ moment(formacao.dataInicio).format("DD/MM/YYYY") }}
                ...
                <div class="p-1 fw-400 text-[7pt]">
                    ...
                    <div v-for="(encontro, index) in formacao.encontros" :key="index">
                        ...
                        <div v-if="inscEncontros[encontro.id] != undefined"
                            ...
                            <div>
                                <span class="ml-2 textx-[10pt] fw-200">Horário:</span>
                                <span class="px-1 rounded text-[10pt] fw-500 text-slate-900">
                                    {{ encontro.horaInicio }} às {{ encontro.horaTermino }}
                                </span>
                            </div>
                            <div class="ml-1 h-[14px] flex items-center rounded-lg bg-gray-200 text-green-800 pr-1">
                                <div class="rounded-lg   bg-gray-300 -mt-[1px]">
                                    <span class="iconify text-[10pt]" data-icon="ic:check"></span>
                                </div>
                                <span class="ml-1 text-[8pt] fw-500">inscrito</span>
                            </div>
                            <div class="pt-[1px]" v-if="inscEncontros[encontro.id].area != ''">
                                <badgearea :sigla="inscEncontros[encontro.id].area" />
                            </div>
                            <div class="flex-1 ml-2 flex justify-center">
                                <div>
                                    <q-btn @click="acessarSala(inscEncontros[encontro.id],encontro.id)" class="px-2 py-1 leading-3"
                                        size="5px" outline>
...
import badgearea from "../utils/badge-area.vue"
...
export default {
    ...
    computed: {
        ...
        inscEncontros() {
            ...
            for (let i in this.formacao.inscEncontros) {
                ...
                let sala = this.formacao.inscEncontros[i].sala
                let elem = { id, area, sala }
                //if (par.length > 1) {
                //    elem.area = par[1]
                //}
    ...
    methods: {
        ...
        acessarSala(inscricao,encontroID) {
            console.log(inscricao);
            console.log("this.formacao",this.formacao);
            let id = "sala" + inscricao.sala + ":" + this.formacao.id + ":" + encontroID + ":" + inscricao.area
            console.log(id);
